 ...
 */
var getCursorPos = function getCursorPos(ev) {
  return {
    x: ev.clientX,
    y: ev.clientY
  };
...
var Cursor = /*#__PURE__*/function () {
   ...
   * @param {String} triggerSelector - Trigger the cursor enter/leave method on this selector's returned elements. Default is all <a> and elements with class 'ceffect'.
   */
  function Cursor(Dom_elems) {
    var _this = this;
    var triggerSelector = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'a, .ceffect';
    ...
    _toConsumableArray(this.DOM.elements).forEach(function (el) {
      return _this.cursorElements.push(new CursorElement(el));
    });
    _toConsumableArray(document.querySelectorAll(triggerSelector)).forEach(function (el) {
...
var CursorElement = /*#__PURE__*/function () {
  ...
  function CursorElement(DOM_el) {
    ...
    // Scales value when entering an element with specified selector
    _defineProperty(this, "radiusOnEnter", 40);
    // Opacity value when entering an element with specified selector
    _defineProperty(this, "opacityOnEnter", 1);
    // radius
    ...
    _defineProperty(this, "primitiveValues", {
      turbulence: 0
    });
    this.DOM.el = DOM_el;
    ...
    var _onMouseMoveEv = function onMouseMoveEv() {
      // Set up the initial values to be the same
      _this2.renderedStyles.tx.previous = _this2.renderedStyles.tx.current = cursor.x - _this2.bounds.width / 2;
      _this2.renderedStyles.ty.previous = _this2.renderedStyles.ty.previous = cursor.y - _this2.bounds.height / 2;
      // Show it
      _this2.DOM.el.style.opacity = _this2.opacity;
      // Start rAF loop
      requestAnimationFrame(function () {
        return _this2.render();
      });
      // Remove the initial mousemove event
      window.removeEventListener('mousemove', _onMouseMoveEv);
    };
    window.addEventListener('mousemove', _onMouseMoveEv);
  }
   ...
   */
  return _createClass(CursorElement, [{
    key: "enter",
    value: function enter() {
...
var customCursor = new Cursor(document.querySelectorAll('.cursor'));
